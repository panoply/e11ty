...
function exists(path) {
  return __async(this, null, function* () {
    try {
      yield (0, import_promises.access)(path);
      return true;
    } catch (e) {
      return false;
    }
...
function write(filePath, data) {
  return __async(this, null, function* () {
    try {
      ...
      if (!exist) {
        yield (0, import_promises.mkdir)(dir, { recursive: true });
      }
      yield (0, import_promises.writeFile)(filePath, data);
    } catch (err) {
      throw new Error(err);
    }
  });
...
function fusion(eleventyConfig, options) {
  ...
  function FuseJson(fileName) {
    return __async(this, null, function* () {
      ...
      const parse = import_marked.default.lexer(read.toString());
      const frontmatter = parse[0].type === "hr" ? parse.splice(0, 2).map(({ raw }) => raw).join("\n") : null;
      if (frontmatter === null) return;
      const data = (0, import_gray_matter.default)(frontmatter).data;
